...
% 'noun(Input, Rest) :- terminal (boy, Input, Rest). è la stessa cosa di:'
% 'noun([boy|Rest], Rest).'
...
% Regole per articoli, nomi e verbi
article --> [il].
article --> [un].
...
test_sentence(X) :-
    phrase(sentence, X),
    write('La frase è grammaticalmente corretta!').
% 'Avrei potuto scivere sentence ?- sentence([il, gatto, cattura, il, topo], []) invece del predicato phrase/2' */
